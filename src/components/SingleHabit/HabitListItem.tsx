    ...
    <div className="flex items-center justify-between p-2 bg-background border-b border-border h-[100px] transition-all duration-300">
      ...
      {!isCollapsed ? (
        <div className="flex flex-col gap-y-1 items-center w-14">
          ...
          <HabitActions onEdit={() => onEdit(habit)} onDelete={() => setIsDeleteOpen(true)} />
        </div>
      ) : (
      ...
      {isDesktop ? (
        <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                <DeleteConfirmation habitName={habit.name} onCancel={() => setIsDeleteOpen(false)} onConfirm={handleConfirmDelete} />
              </DialogDescription>
            </DialogHeader>
          </DialogContent>
        </Dialog>
        ...
        <Drawer open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
          <DrawerContent>
            <DrawerHeader>
              <DrawerTitle className='text-center'>Confirm Deletion</DrawerTitle>
              <DrawerDescription>
                <DeleteConfirmation habitName={habit.name} onCancel={() => setIsDeleteOpen(false)} onConfirm={handleConfirmDelete} />
              </DrawerDescription>
            </DrawerHeader>
